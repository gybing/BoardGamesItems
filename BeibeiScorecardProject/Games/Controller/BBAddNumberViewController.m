...
@interface BBAddNumberViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@end
@implementation BBAddNumberViewController
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)customAddAndDelete:(BOOL)isAdd{
    if (isAdd) {
        self.addBtn.backgroundColor = SMColorFromRGB(0xeb5300);
        ...
        self.deleteBtn.backgroundColor = SMColorFromRGB(0xeb5300);
...
{
    BBAddNumberTableViewCell * cell = [BBAddNumberTableViewCell cellWithTableView:tableView];
    @weakify(self);
    if (indexPath.row == 0) {
...
-(BBJSFastLoginModel *)model
{
    if (!_model) {
        _model = [[BBJSFastLoginModel alloc]init];
    }
    return _model;
}
@end
